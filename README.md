...
This module may be used as a library integrated with other Python projects, or as a standalone command-line tool. Here we describe its use as a Python library.
...
In the case of the `vector` input type, the output will contain a half-open start and end index and the aggregated score over that index (`[start, end)`). The difference of these indices will equal the `window_bins` parameter (e.g., 125 in current usage cases). Here are the column names for this output:
...
Once an interval is selected to be filtered, its score value can be its original score (whatever the score is at the index in the vector, or bin in the bedgraph file), or an *aggregated* value calculated from applying a function on scores over the filtered window. 
...
In the case of the `percentile` aggregation method, the `percentile` parameter may be specified as a value between 0 and 1. Its default is 0.95; however, another value may be specified. Using 0.5, for example, would be equivalent to applying the `median` function.
...
The `Rolling*` statistics are the aggregation function values over each window. The `OriginalIdx` and `MethodIdx` values are indices indicating the starting bin index and the index used for window selection.
